...
class lumise_router {
	...
	public function __construct( $lumise_page = '', $load_assets = true ) {
		...
		$this->menus        = $lumise->apply_filters(
			'admin_menus',
			array(
				'dashboard' => array(
		...
		if ( empty( $lumise_page ) && isset( $_REQUEST['lumise-page'] ) ) {
			$this->_lumise_page = sanitize_text_field( wp_unslash( $_REQUEST['lumise-page'] ) );
		} else {
	...
	public function check_caps() {
		...
		global $lumise;
		$page = isset( $_GET['lumise-page'] ) ? sanitize_text_field( wp_unslash( $_GET['lumise-page'] ) ) : '';
	...
	public function update_notice() {
		...
		$lpage = isset( $_GET['lumise-page'] ) ? sanitize_text_field( wp_unslash( $_GET['lumise-page'] ) ) : '';
		...
		<div class="lumise_container">
			<div class="lumise-col lumise-col-12">
				<div class="lumise-update-notice top">
					<a href="https://www.lumise.com/changelogs/<?php echo esc_attr( $lumise->connector->platform ); ?>?utm_source=client-site&utm_medium=text&utm_campaign=update-page&utm_term=links&utm_content=<?php echo esc_attr( $lumise->connector->platform ); ?>" target=_blank>Lumise <?php echo esc_attr( $this->check_update->version ); ?></a> 
					<?php echo esc_html( $lumise->lang( 'is available' ) ); ?>! 
					<a href="<?php echo esc_url( $lumise->cfg->admin_url . 'lumise-page=updates' ); ?>"><?php echo esc_html( $lumise->lang( 'Please update now' ) ); ?></a>.
				</div>
			</div>
		</div>
			<?php
	...
	public function display() {
		...
		if ( $this->_allow && is_file( $page ) ) {
			...
			if ( ! $this->_allow && is_file( $page ) ) {
				echo esc_html( $lumise->lang( 'Sorry, you are not allowed to access this page.' ) );
			} else {
				echo esc_html( $lumise->lang( 'File not found' ) ) . ': <i>' . esc_html($page) . '</i>';
			}
			echo '</div>';
			echo '</div>';
			echo '</div>';
